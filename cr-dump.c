...
int cr_dump_tasks(pid_t pid)
{
	...
	if (init_stats(DUMP_STATS))
        ...
        // <underscore> load some criu libraries..
	if (cr_plugin_init(CR_PLUGIN_STAGE__DUMP))
		goto err;

        // <underscore> initialize kernel data structure.
	if (kerndat_init())
		goto err;

        // <underscore> loads some kind of inode reverse mapping cache.
	if (irmap_load_cache())
		goto err;

        // <underscore> does some low level serious shit about the CPU status.
	if (cpu_init())
		goto err;

        // <underscore> initialization of some data that is automatically
        // generated by criu. I did not understand its purpose tough.
	if (vdso_init())
		goto err;

        // <underscore> parse cgroups information. 
	if (parse_cg_info())
		goto err;
	...
	if (write_img_inventory())
        ...
        // <underscore> writes CPU info. (pb_write_one)
	 ...
	 */
        ...
        // <underscore> I think all these collect_* methods are just analyzing
        // the target process and getting the necessary information for the 
        // dump.
	if (collect_pstree(pid))
		goto err;
